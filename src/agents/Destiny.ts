...
export class Destiny extends Agent {
    ...
    private modelMode: 'online' | 'offline' = 'online';
     ...
     */
    initialize(schedule: Schedule, characters: Record<string, Character>, scheduler?: Scheduler, modelMode: 'online' | 'offline' = 'online') {
        this.schedule = schedule;
        this.characters = characters;
        this.scheduler = scheduler || null;
        this.modelMode = modelMode;
        console.log(`Destiny: Initialized (Mode: ${this.modelMode})`);
    }

    // ... prepareAllResponses, handleTask, getMovementRequests, generateSpontaneousEvent, updatePositions, getTargetEvent, timeToMinutes, addMinutes, recordWitnessedEvent ...
...
Return ONLY the dialogue, no quotes, no character name prefix.`;
        ...
        try {
            ...
            if (this.modelMode === 'offline') {
                response = await generate(prompt, (_status) => { });
            } else if (this.genAI) {
...
Format: Return ONLY a JSON array of 3 strings, like ["response1", "response2", "response3"]`;
        ...
        try {
            ...
            // Parse JSON array (handle optional markdown blocks from LLM)
